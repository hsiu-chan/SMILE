from ..Base64Converter import path_to_base64
from ..Polygon import PolygonSet,Polygon
...
class SMILE:
    def __init__(self,input_path,out_dir):
        ...
        #####GRIDS######
        self.grid_len=[15,5] #格點距離
        self.grid=[]

        try:
            self.find_mouse()
            self.grid=Polygon(self.mouse).expand(3).gen_grid(self.grid_len)
        except:
            pass
    ...
    def find_all_tooth(self):
        ...
        for pt in self.grid:
            ...
            try:
                mask=Polygon.mask_to_pol(mask)
            except:
                continue
            
            #print(score,mask.area)
            

            h=self.boximg.shape[0]
            w=self.boximg.shape[1]
